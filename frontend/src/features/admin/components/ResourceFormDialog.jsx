...
const imageField = z
  ...
  .refine((file) => !file || ['image/jpeg', 'image/png'].includes(file.type), 'La imagen debe ser jpg o png.');
...
export function ResourceFormDialog({ open, onOpenChange, resourceType, categories, initialData, onSubmit, isPending }) {
  ...
  }, [form, initialData, resourceType]);

  const imageFile = useWatch({ control: form.control, name: 'imagen' });
  const previewFileUrl = useMemo(() => (imageFile instanceof File ? URL.createObjectURL(imageFile) : null), [imageFile]);
  const previewUrl = (previewFileUrl || initialData?.imagenes?.[0] || initialData?.imagen || null)?.replace(/[[\]"]/g, '');
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-full overflow-hidden sm:max-w-3xl">
        <DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="max-w-full space-y-6 overflow-hidden">
