...
export default function MapSidebar({ marker, onClose }) {
  ...
  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 md:left-auto md:right-6 md:translate-x-0 w-[calc(100%-2rem)] md:w-80 z-[400] transition-all duration-300 pointer-events-auto">
      <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 dark:border-slate-700/50 overflow-hidden flex flex-col">
        {/* Header Image */}
        ...
        <div className="p-4 flex flex-col gap-3">
          ...
          <div className="flex items-center gap-1.5 text-sm text-yellow-500 font-medium h-4">
            <Star size={14} className="fill-current text-yellow-400" />
            <span>{marker.rating || '4.5'}</span>
          ...
          {/* Action Button */}
          <button className="w-full mt-2 bg-brand-500 hover:bg-brand-600 text-white text-sm font-semibold py-2 rounded-xl transition-colors shadow-lg shadow-brand-500/20">
            Ver detalles
          </button>
        </div>
      </div>
